    ...
    bool Container::handleEvent(sf::Event& event)
    {
        ...
        else if (event.type == sf::Event::MouseButtonPressed)
		{
			...
			if(event.mouseButton.button == sf::Mouse::Right)
			{
				...
				if(widget != nullptr)
				{
					...
					widget->rightMousePressed(static_cast<float>(event.mouseButton.x), static_cast<float>(event.mouseButton.y));
					caught = true;
				}
				else // The mouse didn't went down on a widget, so unfocus the focused widget
					unfocusWidgets();
        ...
        else if (event.type == sf::Event::MouseButtonReleased)
        {
			...
			if(event.mouseButton.button == sf::Mouse::Right)
			{
				...
				Widget::Ptr widget = mouseOnWhichWidget(static_cast<float>(event.mouseButton.x), static_cast<float>(event.mouseButton.y));
				if(widget != nullptr)
					widget->rightMouseReleased(static_cast<float>(event.mouseButton.x), static_cast<float>(event.mouseButton.y));
