...
namespace tgui
{
	...
	void Panel::rightMousePressed(float x, float y)
	{
		if(mouseOnWidget(x, y))
		{
			m_MouseDown = true;

			if(!m_CallbackFunctions[RightMousePressed].empty())
			{
				m_Callback.trigger = RightMousePressed;
				m_Callback.mouse.x = static_cast<int>(x - getPosition().x);
				m_Callback.mouse.y = static_cast<int>(y - getPosition().y);
				addCallback();
			}
		...
		Container::rightMousePressed(x, y);
	}

	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	void Panel::rightMouseReleased(float x, float y)
	{
		if(mouseOnWidget(x, y))
		{
			if(!m_CallbackFunctions[RightMouseReleased].empty())
			{
				m_Callback.trigger = RightMouseReleased;
				m_Callback.mouse.x = static_cast<int>(x - getPosition().x);
				m_Callback.mouse.y = static_cast<int>(y - getPosition().y);
				addCallback();
			}
			...
			if(m_MouseDown)
			{
				if(!m_CallbackFunctions[RightMouseClicked].empty())
				{
					m_Callback.trigger = RightMouseClicked;
					m_Callback.mouse.x = static_cast<int>(x - getPosition().x);
					m_Callback.mouse.y = static_cast<int>(y - getPosition().y);
					addCallback();
				}
		...
		Container::rightMouseReleased(x, y);
	}
